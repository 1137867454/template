...
const corporation = {
   ...
   */
    ...
    // 修改部门 post /corporation/updLeader

    updateLeader({state, dispatch}, data) {
      return new Promise( (resolve, reject) => {
        let url = `/corporation/updLeader`;
        let methods = 'post';
        fx.setConnect({url, methods, data})
        .then(res => {
          resolve(1);
    ...
    // 部门查询 get /corporation/getLeaderPageList
    ...
    getLeaderList({state, commit}, page) {
      ...
      let url = `/corporation/getLeaderPageList?corpId=${page.corpId}&pageSize=${page.pageSize}&pageNum=${page.currentPage}&name=${page.name}` ;
      fx.setConnect({url})
      .then(res => {
