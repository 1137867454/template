  ...
  <el-form  style="margin-left:60px;"
      ...
      >
        ...
        <el-form-item label-width="0" style="">
          <el-button
            type="primary"
            @click="addPermission('addPermission', user)"
          >
            确 定
          </el-button>
...
export default {
  ...
  methods: {
    ...
    getKey(obj, v, t) {
      ...
      this.permission.arr = Array.from(new Set(this.permission.arr));
      // console.log(obj);
      // console.log(v);
      // console.log(t);
    },
    ...
    // 添加与修改
		addPermission(form, user) {
      this.$refs['tree'].
			this.$refs[form].validate(valid => {
				if(valid) {
          console.log(user)
          if(this.updateVisible){ //账号添加、修改判别
            this.$store.dispatch('addPermission', user)
            .then(res => {
