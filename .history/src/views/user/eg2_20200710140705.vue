  ...
  methods: {
    nodeKey(x,y) {
      ...
      if(!x.menu && !x.children){
        ...
        if(y.halfCheckedNodes.length) {
            y.halfCheckedNodes.forEach(v => {
              if( !v.menu ){
                console.log(v)
                this.menuArr.push(v.id)
              }else{
                // this.menuArr.pop();
              }
            })
          ...
          var idx = this.menuArr.indexOf(x.parentId);
          this.menuArr.splice(idx, 1);
          // let branch = localStorage.getItem('branch');
          // var id = branch.find(bra => {
          //   return bra.id == x.id;
          // })
          // console.log(id);
        }
        ...
        this.menuArr = Array.from(new Set(this.menuArr));
      }
      if(!y.halfCheckedNodes.length){
        console.log(this.menuArr)
        this.menuArr.forEach(a => {
          let i = this.permission.permissionId.indexOf(a);
          this.permission.permissionId.splice(i,1);
          // console.log(i)
        })
      }
    },
    getKey(obj, v, t) {
      ...
      // console.log('..............')
      // console.log(v, t)
      var len = this.permission.permissionId.length;
      console.log("length: "+len)
      if(v) {
        console.log(obj.id)
        this.permission.permissionId.push(obj.id)
        console.log("push_length: "+len)
      }else{
        // 过滤取消选中的标签id
        console.log(obj)
        var i = this.permission.permissionId.indexOf(obj.id)
        this.permission.permissionId.splice(i,1);
        // console.log("length: "+ this.permission.permissionId.length)
        //   if(len == this.permission.permissionId.length){
        //     this.permission.permissionId.pop();
        //   }
      }
